...
ax.set_xlim(-20, 20)  # пределы изменения переменной x
ax.set_ylim(-20, 20)  # пределы изменения переменной y
ax.set_aspect('equal')


def update(t_cur):
    x2 = 16*np.sin(t_cur)**3
    y2 = 13*np.cos(t_cur) - 5*np.cos(2*t_cur) - 2*np.cos(3*t_cur)- np.cos(4*t_cur)
    x1.append(x2)
    y1.append(y2)
...
ani.save('les7_task2_heart.gif', writer='pillow')