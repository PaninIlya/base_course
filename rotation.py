...
#Параметры орбиты маленькой сферы
orbit_radius = R + R_small + 0.9  
orbit_speed = 0.03
orbit_height = start_z - 3  
...
#Глобальная переменная для угла орбиты
orbit_angle = 0
...
def spinnig_func(frame):
    ...
    #Вычисляем позицию маленькой сферы на орбите
    small_x = orbit_radius * np.cos(orbit_angle)
    small_y = orbit_radius * np.sin(orbit_angle)
    small_z = orbit_height
    ...
    #Рисуем орбиту маленькой планеты
    orbit_theta = np.linspace(0, 2 * np.pi, 100)
    orbit_x = orbit_radius * np.cos(orbit_theta)
    orbit_y = orbit_radius * np.sin(orbit_theta)
    orbit_z = np.full_like(orbit_theta, orbit_height)
    ax.plot(orbit_x, orbit_y, orbit_z, color='gray', linewidth=1, alpha=0.5)
    ...
    ax.set_title('Photometry - Маленькая планета движется по орбите', color='black')

    max_bound = max(orbit_radius, start_x, start_y, start_z) + 2
